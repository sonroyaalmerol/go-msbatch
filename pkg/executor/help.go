...
func CommandNames() []string {
	...
	for name := range tools.ToolHelp {
		names = append(names, name)
	}
	return names
}

// CommandHelp returns the help text for a CMD/batch command, or an empty string.
// Builtin command help is defined here; tool help is sourced from the tools package.
func CommandHelp(name string) string {
	lower := strings.ToLower(name)
	if h := builtinHelp[lower]; h != "" {
		return h
	}
	return tools.Help(lower)
}
